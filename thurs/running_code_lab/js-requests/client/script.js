...
sayHelloButton.addEventListener('mouseout',colorBack)
...
const repeatMyParam = () => {
    let temp = document.querySelector("#repeat-text")
    axios.get(`http://localhost:3000/repeat/${'Some Param'}`).then(res =>{
        console.log(res.data)
        temp.textContent = res.data
...
// CODE HERE
const prob = () => {
    axios.get(`http://localhost:3000/query-test?test=${'test'}`
    ).then(res => {
        let temp = document.querySelector("#repeat-text")
        if(typeof(res.data) === Object){
            temp.textContent = res.data.message
        }
        else(
            temp.textContent = res.data
        )
...
let query = document.querySelector('#query-button')
query.addEventListener('click',prob)
...
const createFood = (event) => {
    ...
    axios.post('http://localhost:3000/food',body).then(res =>{
        ...
        header.innerHTML = `Your Food List: ${res.data}`
        foodList.innerHTML = ''
